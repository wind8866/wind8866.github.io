...
</code></pre></div><h3 id="fresh-reqheaders-resheaders"><a href="#fresh-reqheaders-resheaders" class="header-anchor">#</a> fresh(reqHeaders, resHeaders)</h3> <p>Check freshness of the response using request and response headers.</p> <p>When the response is still &quot;fresh&quot; in the client's cache <code>true</code> is<br>
returned, otherwise <code>false</code> is returned to indicate that the client<br>
cache is now stale and the full response should be sent.</p> <p>When a client sends the <code>Cache-Control: no-cache</code> request header to<br>
indicate an end-to-end reload request, this module will return <code>false</code><br>
to make handling these requests transparent.</p> <h2 id="known-issues"><a href="#known-issues" class="header-anchor">#</a> Known Issues</h2> <p>This module is designed to only follow the HTTP specifications, not<br>
to work-around all kinda of client bugs (especially since this module<br>
typically does not recieve enough information to understand what the<br>
client actually is).</p> <p>There is a known issue that in certain versions of Safari, Safari<br>
will incorrectly make a request that allows this module to validate<br>
freshness of the resource even when Safari does not have a<br>
representation of the resource in the cache. The module<br> <a href="https://www.npmjs.com/package/jumanji" target="_blank" rel="noopener noreferrer">jumanji<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> can be used in<br>
an Express application to work-around this issue and also provides<br>
...
server<span class="token punctuation">.</span><span class="token function">listen</span><span class="token punctuation">(</span><span class="token number">3000</span><span class="token punctuation">)</span>
    ...
    <script src="/assets/js/app.2e3db3d9.js" defer></script><script src="/assets/js/2.65dda3ac.js" defer></script><script src="/assets/js/114.a826c59d.js" defer></script>
  </body>
</html>
